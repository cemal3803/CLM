...
public class CNF_API {
	...
	static ArrayList<String> run(String input){
		char[] combi = new char[1000];

		Pattern p = Pattern.compile("[0-9]+") ;  
		Matcher m = p.matcher(input) ;  
		char act = 'A';
		while (m.find()) {
			combi[Integer.parseInt(m.group())] = act;
			input = input.replaceAll(m.group(), ""+act);
			act++;
			m = p.matcher(input) ;  
		}
		...
		all.add(five.get_result());
		  ...
		  for(int i = 0; i < all.size(); i++)
		  {
				...
				String finalLine = all.get(i);

				p = Pattern.compile("[A-Z]+") ;  
				m = p.matcher(finalLine) ;  

				while (m.find()) {
					int c = 0;
					
					while(combi[c] != m.group().charAt(0) && c != combi.length)
						c++;

					finalLine = finalLine.replaceAll(m.group(), c+"");
					act++;
				}
		...
		String s = newList.get(newList.size()-1);
		s = s.replaceAll("\\(", "");
		s = s.replaceAll("\\)", "");
		s = s.replaceAll("or", "");

		lastList = new ArrayList<>(Arrays.asList(s.split("and ")));
